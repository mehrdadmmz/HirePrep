# HirePrep — AI-powered mock interviews (voice, realtime)
...
![Gemini](https://img.shields.io/badge/Google-Gemini-4285F4?logo=google&logoColor=fff)
![Vapi](https://img.shields.io/badge/Vapi-Voice_Agent-6B4EFF)
![shadcn/ui](https://img.shields.io/badge/shadcn%2Fui-Components-111827)
![Zod](https://img.shields.io/badge/Zod-Schema_Validation-0F766E)

> Piloted with **50+ SFU students**, improving interview performance scores by **~20%** through realtime, voice-based practice and structured AI feedback.

---

## Overview

**HirePrep** is a full-stack mock interview platform that lets you:
- spin up **role-specific interviews** (e.g., “Full-stack — TS/React/Node”),
- talk to an **AI voice interviewer** in real time (Vapi),
- and receive **structured feedback** (Gemini) with strengths, areas to improve, and a total score.

It’s built for quick reps: generate questions, speak your answers, get feedback, iterate.
...
- **Backend/Services:** Firebase Auth, Firestore, Edge/Route Handlers  
- **AI:** Google **Gemini** (evaluation, question gen), **Vapi** (voice agent)  
- **Validation:** **Zod** (forms, API payloads)

---

## Features

- **Auth & Sessions:** Email/password with Firebase Auth; SSR-friendly session helpers.  
- **Interview Generation:** Role, level, tech stack, and question count → curated prompts to Gemini.  
- **Voice Interviews:** Realtime conversation with a **Vapi** agent; transcripts are persisted.  
- **AI Feedback:** Gemini scores **Communication**, **Technical Knowledge**, **Problem-Solving**, **Role Fit**, **Confidence** (0–100) + summary, strengths, improvements.  
- **Dashboard:** List, filter, and revisit interviews; re-take with one click.  
- **Modern UI/UX:** Tailwind + shadcn/ui components, dark-mode friendly.  
- **Type-safe:** Zod schemas shared across UI and API.
...
- Vapi account (web token + workflow)

**Cloning the Repository**

```bash
git clone https://github.com/mehrdadmmz/HirePrep
cd HirePrep
```
...
/app
  ...
  /(root)
    ...
    /interview/[id]
      /feedback
  /api
    /vapi/generate    # question generation
    /feedback         # evaluation endpoint
/lib
  /actions            # server actions (Gemini, Firestore)
  /firebase           # client/server firebase initializers
  /validators         # Zod schemas
  /utils.ts
/components          # shadcn/ui wrappers and custom UI
/constants           # mappings, covers, config
...
## Privacy & Safety
- User accounts handled by Firebase Auth; Firestore rules restrict access per user.
- Only interview metadata, transcripts, and feedback are stored.
- API keys are server-side where possible; never commit .env.local.
